...
namespace {
#define TEST_FAIL_MSG "utility.cpp : Unit test failed."
...
TEST(enum_utility, basics) {
	...
	static_assert(std::is_same<ret1_t, uint8_t>::value, TEST_FAIL_MSG);

	using ret2_t = decltype(fea::to_underlying(e2{}));
	static_assert(std::is_same<ret2_t, uint16_t>::value, TEST_FAIL_MSG);

	using ret3_t = decltype(fea::to_underlying(e3{}));
	static_assert(std::is_same<ret3_t, int>::value, TEST_FAIL_MSG);
