...
namespace {
#define test_failed_msg "unsigned_map.cpp : Unit test failed."
...
TEST(unsigned_map, ids) {
	...
	fea::unsigned_map<my_id, int> map2{ map };
	fea::unsigned_map<my_id, int> map_ded{ map };
	fea::unsigned_map<my_id, int> map3{ std::move(map_ded) };
	...
	map = fea::unsigned_map<my_id, int>({
			{ my_id(0), 0 },
			{ my_id(1), 1 },
			{ my_id(2), 2 },
	});
	map2 = fea::unsigned_map<my_id, int>({
			{ my_id(3), 3 },
			{ my_id(4), 4 },
			{ my_id(5), 5 },
	});
	map3 = fea::unsigned_map<my_id, int>({
			{ my_id(6), 6 },
			{ my_id(7), 7 },
			{ my_id(8), 8 },
	});
	...
	{
		fea::unsigned_map<my_id, int> map1_back = map;
		fea::unsigned_map<my_id, int> map2_back{ map2 };
		fea::unsigned_map<my_id, int> map3_back{ map3 };
	...
	map2 = fea::unsigned_map<my_id, int>(map.begin(), map.end());
